...
sub maybe_my {
	...
	return text_info($text, 'maybe_my');
    }
}
...
sub unop {
	...
	return {
	    text => $self->maybe_parens_unop($name, $kid, $cx, $op),
	    type => 'unop',
	};
    } else {
...
sub pp_readline {
    ...
    return "<" . $self->deparse($kid, 1, $op) . ">" if is_scalar($kid);
...
sub loopex {
	...
	(my $kid = $self->deparse($op->first, 7, $op)) =~ s/^\cS//;
	$name .= " $kid";
    }
...
sub range {
    ...
    return text_info $self->maybe_parens("$left $type $right", $cx, 9);
}
...
sub logop {
	...
	$texts = [$rhs->{text}, $highop, $lhs->{text}];
...
sub rv2gv_or_string {
	...
	return text_info $quoted ? $name : "*$name";
    }
    else {
...
sub listop {
	...
	$first = text_info(sprintf("%#o", $first));
    }
...
sub pp_list {
    ...
    if ($local) {
	$info->{texts} = [$local, '(', join(", ", @args), ')'];
	$info->{text} = join('', $info->{texts});
...
sub pp_padsv {
    my $self = shift;
    my($op, $cx) = @_;
    return { text => $self->maybe_my($op, $cx, $self->padname($op->targ)) };
}
...
sub pp_gvsv {
    ...
    my $ret = $self->maybe_local($op, $cx, $self->stash_variable("\$",
								 $self->gv_name($gv), $cx));
    return $ret;
}
...
sub pp_gv {
    ...
    my($op, $cx) = @_;
    my $gv = $self->gv_or_padgv($op);
    return text_info($self->gv_name($gv));
}
